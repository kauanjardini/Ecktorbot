...
const { help } = require('./kauan/help')
const {logos} = require('./kauan/logos')
const {help1} = require('./kauan/help1')
const { modapk } = require('./kauan/modapk')
const { pack } = require('./kauan/pack')
const { cv1} = require('./comm4nd')
const { outros } = require('./kauan/outros')
const { vipmenu } = require('./kauan/vipmenu')
const { destrava } = require('./kauan/destrava')
const { menu2 } = require('./kauan/menu2')
const { destrava2 } = require('./kauan/destrava')
const { gbin } = require('./kauan/gbin')
const { gpessoa } = require('./kauan/gpessoa')
const { chentai } = require('./kauan/chentai')
const { gcpf } = require('./kauan/gcpf')
const { addsay } = require('./kauan/addsay')
const { listsay } = require('./kauan/listsay')
const { addfoto } = require('./kauan/addfoto')
...
const { cekvip } = require('./kauan/cekvip')
const { TobzApi } = JSON.parse(fs.readFileSync('./database/json/apikey.json'))
const { VthearApi } = JSON.parse(fs.readFileSync('./database/json/apikey.json'))
const { daftarvip } = require('./kauan/daftarvip')
const { iklan } = require('./kauan/iklan')
const { daftatvip } = require('./kauan/daftarvip')
...
apibarbar = setting.apibarbar
apitobz = setting.apitobz
apivhtear = setting.apivhtear
const BotName = 'ECKTOR bot'; 
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Kauan\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=554491150998:+55 44 91150998\n' //Nomor whatsapp kamu
            + 'END:VCARD'
blocked = []
limitawal = '999999999'
cr = '*kauan*'           
...
async function starts() {
	const kauankkk = new WAConnection()
	kauankkk.logger.level = 'warn'
	console.log(banner.string)
	kauankkk.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' escaneie o qr code com o whatsapp web '))
	})

	fs.existsSync('./whii.json') && kauankkk.loadAuthInfo('./whii.json')
	kauankkk.on('connecting', () => {
		start('2', 'Espere só um pouquinho kkk...')
	})
	kauankkk.on('open', () => {
		success('2', 'o bot ja está ativo')
	})
	await kauankkk.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./whii.json', JSON.stringify(kauankkk.base64EncodedAuthInfo(), null, '\t'))

	kauankkk.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await kauankkk.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await kauankkk.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `Olá @${num.split('@')[0]}👋👋👋\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste daqui👉👈️`
				let buff = await getBuffer(ppimg)
				kauankkk.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				kauankkk.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await kauankkk.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde demais kkkkk `
				let buff = await getBuffer(ppimg)
				kauankkk.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	kauankkk.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ECKTOR BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Kauan? Este é um recurso especial para o Kauan ❌',
					ownerB: '[❗] Kauan? Este é um recurso especial para o Kauan ❌',
			...
			const botNumber = kauankkk.user.jid
			const ownerNumber = ["554491150998@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"554491150998@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["554491150998@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["554491150998@s.whatsapp.net"]//ubah nomor lo
			const premium = ["554491150998@s.whatsapp.net","5511943370311@s.whatsapp.net"] 
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await kauankkk.groupMetadata(from) : ''
		    ...
		    const isUser = kauankkk.contacts[sender] != undefined ? kauankkk.contacts[sender].vname || kauankkk.contacts[sender].notify : undefined
			pushname = kauankkk.contacts[sender] != undefined ? kauankkk.contacts[sender].vname || kauankkk.contacts[sender].notify : undefined
		    pushname2 = kauankkk.contacts[sender] != undefined ? kauankkk.contacts[sender].vname || kauankkk.contacts[sender].notify : undefined
			...
			const reply = (teks) => {
				kauankkk.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendImage = (teks) => {
		    kauankkk.sendMessage(from, teks, image, {quoted:mek})
		    }
			const sendMess = (hehe, teks) => {
				kauankkk.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? kauankkk.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
              //function leveling
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			kauankkk.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./kauan/darkjokes.js');
			...
			switch(command) {
				...
				case 'menu':
		             if (args.length < 1) return reply ('Olá ${pushname} espero q vc esteva bem! seja bem vindo ao meu menuzin tá👉👈😗')
		             break
				
				case 'menu':
				        
                    kauankkk.sendMessage(from, `Velocidade do bot: *${latensi.toFixed(4)} _Segundos_*`, text, { quoted: mek})
                    break
				...
				case 'menu':				      
					   ...
					   putagg = await getBuffer(`https://i.ibb.co/h7B7rxT/menu.jpg`)
					   menuimg = fs.readFileSync('./assets/menuimg.jpg')
                       kauankkk.sendMessage(from, putagg, image, {quoted: mek, caption: help(prefix, pushname)})
                    break
                 
            	case 'help1':
				case 'menu1':
					kauankkk.sendMessage(from, help1(prefix), text)
					break
					case 'kiss':
				    try {    
						...
						kauankkk.sendMessage(from, bufferv, image, {quoted: mek, caption: 'ezzzz'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						sa = await getBuffer(`https://i.ibb.co/JcSjmNY/IMG-20210107-WA0052.jpg`)
						kauankkk.sendMessage(from, sa, image, {quoted: mek, caption: 'Erro como!!'})
						reply('❌ *ERRO* ❌')
					}
					break
					...
					case 'logos':				      
					   ...
					   menuimg = fs.readFileSync('./assets/menuimg.jpg')
                      kauankkk.sendMessage(from, logos(prefix, time, kyun, pushname), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*logos👉👈*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                    break   
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek})
					break
//_TIPO DE MENSAGENS
...
media = await kauankkk.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${media} -vf reverse -af areverse ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply(`Err: ${err}`)
buffer453 = fs.readFileSync(ran)
kauankkk.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
fs.unlinkSync(ran)
})
break
...
kauankkk.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break   
...
slo = await kauankkk.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
...
tup = await kauankkk.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
gem = await kauankkk.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
bas = await kauankkk.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
break
                    ...
                    case 'fumaca':
					if (!isGroup) return reply(mess.only.group)
					anu = await fetchJson(`https://api-gdr.herokuapp.com/api/smoke?text=${body.slice(8)}`)
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek})
					break
                    ...
                    case 'wolf':
					if (!isGroup) return reply(mess.only.group)
					anu = await fetchJson(`https://api-gdr.herokuapp.com/api/wolf111?text=${body.slice(8)}`)
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek})
					break
                   ...
                   case 'coffee':
					if (!isGroup) return reply(mess.only.group)
					anu = await fetchJson(`https://api-gdr.herokuapp.com/api/coffee?text=${body.slice(8)}`)
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek})
					break
                    ...
                    case 'romantico':
					if (!isGroup) return reply(mess.only.group)
					anu = await fetchJson(`https://api-gdr.herokuapp.com/api/romantic?text=${body.slice(8)}`)
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek})
					break
...
kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: teks.trim()})
break
					...
					reply(mess.wait)
					kauankkk.sendMessage(from, 'Adicionando mensagem à lista com sucesso' , text, { quoted: mek })
					kauankkk.sendMessage(from, addsay(prefix), text, { quoted: mek })
					break
			
				case 'macaco':	
					kauankkk.sendMessage(from, image, macaco)
                    break
					
					case 'addsay':
				    hai = body.slice(8)
						sayrandom.push(hai)
						fs.writeFileSync('./kauan/say.json', JSON.stringify(sayrandom))
                   ...
                   case 'saylist':
					...
					kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": sayrandom}})
					break
				case 'ssweb':
					...
					kauankkk.sendMessage(from, bufferz, image, {quoted: mek})
					break
                case 'ytkomen':
					...
					try {
					pp = await kauankkk.getProfilePicture(ghost)
					} catch {
					...
					kauankkk.sendMessage(from, buffer88, image, {quoted: mek, caption: `*${usnm}* : ${cmn}`})
					break
				case 'walpaperhd':
					...
					kauankkk.sendMessage(from, bufferx, image, {quoted: mek})
					break
			case 'jadian':
                    ...
                    kauankkk.sendMessage(from, sapa)
		       ...
		       case 'figuani':
					...
					kauankkk.sendMessage( from, anu, sticker, {quoted:mek})
					break
					
              case 'modapk':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, modapk(prefix), text, { quoted: mek })
                    break
                   case 'gbin':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, gbin(prefix), text, { quoted: mek })
                    break
                   case 'gpessoa':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, gpessoa(prefix), text, { quoted: mek })
                    break
                   case 'destrava':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, destrava(prefix), text, { quoted: mek })
                    break
                   case 'pack':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, pack(prefix), text, { quoted: mek })
                    break
                   case 'chentai':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, chentai(prefix), text, { quoted: mek })
                    break
                   case 'gcpf':
                    if (!isPremium) return reply(mess.only.premium)
                    kauankkk.sendMessage(from, gcpf(prefix), text, { quoted: mek })
                    break
				case 'ytmp4':
					...
					kauankkk.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
					break
		case 'iri':
			kauankkk.sendPtt(from, './lindy/iri.mp3', {quoted: mek, ptt:true})
			break
				case 'ytmp3':
			    	if (!isPremium) return reply(mess.only.premium)
                   reply(mess.wait)
					...
					kauankkk.sendMessage(from, bufferyyy, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'ytmp':
					if (args.length < 1) return reply('Cadê o url, hum?')
					...
					kauankkk.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, audio, {mimetype: 'audio/mp3', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'game':
				
					anu = await fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
					setTimeout( () => {
					kauankkk.sendMessage(from, '*➸ Responda :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, '_10 Outro segundo…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, '_20 Outro segundo_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, '_30 Outro segundo_…', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, anu.result.soal, text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					break
                case 'quotemaker':
					...
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, image, {caption: 'Nih dah jadi kak', quoted: mek})
					break
				case 'galaxtext':
				...
				kauankkk.sendMessage(from, `${anu.result}`, text, {quoted: mek})
			...
			break
				...
				decoc = `${body.slice(10)}`
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							kauankkk.sendMessage(_.jid, buff, image, {caption: `[Kauan falando]\n\n${body.slice(4)}`})
						}
						reply('Feito')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `「Kauan FALANDO」\n\n${body.slice(4)}`)
						}
						reply('Feito')
					}
					break	
				...
				break 
					case 'hashidentifier':
					  ...
					  kauankkk.sendMessage(from, hasilhash, text, {quoted: mek})
					  await limitAdd(sender)
					  break 
                case 'tahta':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Harta Tahta '+teks})
					break
				case 'testing':
					var gh = body.slice(5)
					var gbl3 = gh.split("|")[0];
				...
				case 'lovemake':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: ' '+love})
					break
				case 'thunder':
					if (args.length < 1) return reply('Cadê o texto, hum')
					thun = body.slice(9)
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: ' '+thun})
					break
                case 'stiltext':
                      ...
                      reply(mess.wait)
                      kauankkk.sendMessage(from, buff, image, {quoted: mek, caption: 'thund ni '+gh})
                      break
                case 'testing':
					var gh = body.slice(9)
					coli1 = gh.split("|")[0];
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'plogo':
					if (args.length < 1) return reply(mess.blank)
					teks = body.slice(7)
				...
				case 'testing2':
					...
					kauankkk.sendMessage(from, party, image, {quoted: mek})
					break
                case 'ninjalogo':
                      if (args.length < 1) return reply('Cadê o texto?')
                      gh = body.slice(11)
                      ...
                      kauankkk.sendMessage(from, buff, image, {quoted: mek})
                      break
		case 'tahta':
                tahta = `${body.slice(7)}`
                  ...
                  break  
                    ...
                    kauankkk.sendMessage(from, hasil, image, {quoted: mek, caption: 'neh...'})
                    await limitAdd(sender)
                    break
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
                ...
                bufferddd = await getBuffer('https://raw.githubusercontent.com/FarhanXCode7/termux-bot-wa/main/src/glitchtext.png')
                 reply(mess.wait)
                buff = await getBuffer(anu.result)
                kauankkk.sendMessage(from, bufferddd, image, {quoted: mek, caption: sck})
                kauankkk.sendMessage(from, buff, video, {mimetype: 'video/mp4', filename: `${anu.format}.mp4`, quoted: mek})
                await limitAdd(sender) 
                break  
				case 'party':
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, buffer, image, {caption: 'Aqui amigo (a)', quoted: mek})
					break
				case 'modoanime':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'rtext':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: tels5})
					break
				case 'water':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
				...
				case 'loli2':
					...
					kauankkk.sendMessage(from, nyeee, image, { caption: 'Oni chan baka!!', quoted: mek })
					break
					case 'xd':				
					...
					kauankkk.sendMessage(from, pineq, image, { caption: '*Pinterest*\n\n*Resultado Pesquisa : '+pinte+'*', quoted: mek })
					break
//@darkYT					
				case 'pokemon':
                    kauankkk.updatePresence(from, Presence.composing) 
					...
					kauankkk.sendMessage(from, pok, image, { quoted: mek })
                    await limitAdd(sender)
					break
              case 'circlesticker':
    ...
    if (isMedia && mimetype == 'image/jpeg') {
      ...
      decryptMedia(quotedMsg).then(mediaData => {
        sharp(mediaData).resize({
          ...
          modifExif(buffer, id, (res) => {
            mediaData = res.toString('base64')
            kauankkk.sendRawWebpAsSticker(from, mediaData)
          })
        })
      })
      ...
      await convertSticker(shape, author, pack, mediaData, type, from).then((res) => kauankkk.sendRawWebpAsSticker(from, res.toString("base64")))
    ...
    break
				...
				case 'semoji':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						kauankkk.sendMessage(from, buffer, sticker)
						fs.unlinkSync(rano)
					})
					break
				case 'firetext':
...
fs.unlinkSync(ran)
				...
				case 'textblue':
					...
					kauankkk.sendMessage(from, vipmenu(prefix) , text, { quoted: mek })
					break
					case 'outros':
					kauankkk.sendMessage(from, outros(prefix) , text, { quoted: mek })
					break
					case 'menu2':
					kauankkk.sendMessage(from, menu2(prefix) , text, { quoted: mek })
					break
				case 'textsky':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(9)
				...
				case 'rize':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'rize chan!!', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'mia':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					kauankkk.sendMessage(from, nye, image, { caption: 'i love you, mia 🥺❤️', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'texteng':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://kauanjardini.sirv.com/sunset.png?text.0.text=${teks}&text.0.position.x=-39%25&text.0.position.y=-10%25&text.0.size=24&text.0.font.family=Yanone%20Kaffeesatz&text.0.font.weight=300&text.0.background.opacity=69`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
				...
				case 'tulis':
					...
					if (anu.error) return reply(anu.error)
					buff = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buff, image, {quoted: mek, caption: mess.success})
                    await limitAdd(sender)
					break
				case 'jogo':
					anu = await fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
					...
					kauankkk.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
                        console.log(res)
                    })
					await limitAdd(sender)
					break
				case 'bloquear':
				 kauankkk.updatePresence(from, Presence.composing) 
				 kauankkk.chatRead (from)
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					kauankkk.blockUser (`${body.slice(7)}@c.us`, "add")
					kauankkk.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
				case 'roletahard':
if(!isGroup) return reply(mess.only.group)
...
await kauankkk.sendMessage(from, `💥O @${mem.split('@')[0]} Não teve sorte💥\n*😈 Agora se prepare para as consequências 😈*`, text, {contextInfo: {"mentionedJid": [mem]}})
await kauankkk.groupRemove(from, [mem])
await kauankkk.sendMessage(from, `👻O @${mem.split('@')[0]} teve sua vida ceifada👻`)
}, 10000)
break	
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
					...
					kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": premium}})
					break
                  case 'qrcode':
					...
					if (!tex) return kauankkk.sendMessage(from, 'Digite um texto/url que deseja criar um código qr', text, {quoted: mek})
					const bufferr = await getBuffer(`https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${tex}`)
					kauankkk.sendMessage(from, bufferr, image, {quoted: mek})
					break
		    	case 'wa.me':
		        case 'wame':
                  kauankkk.updatePresence(from, Presence.composing) 
                  ...
                  kauankkk.sendMessage(from, options, text, { quoted: mek } )
			      break
			      case 'pslogo':
					if (args.length < 1) return reply(mess.blank)
					teks = body.slice(7)
            ...
            case 'onichan':
            case 'bodoh':
                kauankkk.sendMessage(from, buff, './lindy/baka.mp3', audio/mp3, {quoted: mek, ptt:true})
                break
				case 'daftar':
					kauankkk.updatePresence(from, Presence.composing)
					...
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./kauan/user.json', JSON.stringify(user))
						kauankkk.sendMessage(from, `\`\`\`Pendaftaran berhasil dengan SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`Pada ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, bufferjj, document, {mimetype: 'document/pdf', quoted: mek})
					break
                 case 'setdesc':
			    	 if (!isGroup) return reply(mess.only.group)
				     if (!isGroupAdmins) return reply(mess.only.admin)
			   	     if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				     kauankkk.groupUpdateDescription(from, `${body.slice(9)}`)
				     kauankkk.sendMessage(from, 'Descrição alterada com sucesso', text, {quoted: mek})
				     break
			case 'google':
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    console.log(e)
                    kauankkk.sendMessage(from, 'Google Error : ' + e);
                })
                await limitAdd(sender) 
                break 
		case 'bpfont':
			...
			break 
				...
				case 'd':  
					...
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
					kauankkk.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
                case 'travar':
                     const pesan = body.slice(5)
                      if (!isPremium) return reply(mess.only.premium)
                      if (pesan.length > 5000) return kauankkk.sendMessage(from, 'ate 5k de caracteres', msgType.text, {quoted: mek})
                    ...
                    kauankkk.sendMessage('556192247994@s.whatsapp.net', options, text, {quoted: mek})
                    reply('1 trava enviada')
                    break
                    
          		case 'attp':
			if (args.length == 0) return kill.reply(from, mess.noargs() + 'palavras/words/números/numbers.', id)
			await kauankkk.reply(from, mess.wait(), id)
			await kauankkk.get(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(body.slice(6))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				const attp = Buffer.from(response.data, 'binary').toString('base64')
				await kauankkk.sendImageAsSticker(from, attp, { author: config.author, pack: config.pack, keepScale: true })
			})
			break          
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                ...
                kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                kauankkk.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                await limitAdd(sender) 
                break 
            case 'smule':
					if (args.length < 1) return reply('Cadê o url mano?')
					...
					kauankkk.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek, caption: 'Aqui mano'})
					await limitAdd(sender) 	
					break  
				case 'ttp':
					if (args.length < 1) return reply('Cadê o texto, hum?')
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						kauankkk.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
                                        await limitAdd(sender)
					break
                  case 'desbloquear':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				    kauankkk.blockUser (`${body.slice(9)}@c.us`, "remove")
					kauankkk.sendMessage(from, `perintah Diterima, membuka blokir ${body.slice(9)}@c.us`, text)
				    break
                case 'image':
					...
					kauankkk.sendMessage(from, pint, image, { caption: '*Google Image*\n\n*Resultado da pesquisa : '+goo+'*', quoted: mek })
					break
				case 'ban':
					if (!isOwner) return reply(mess.only.ownerB)
					kauankkk.banUser (`${body.slice(7)}@c.us`, "add")
					kauankkk.sendMessage(from, `você foi banido ${body.slice(7)}@c.us`, text)
					break
				case 'playstore':
					kuji = body.slice(7)
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, anu, image, {quoted: mek, caption: capty})
					break
				case 'otagall2':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                case 'pinterest':
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					kauankkk.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST*\n\*Resultado da pesquisa* : *${tels}*`})
                    await limitAdd(sender)
					break
                case 'darkjokes':
				 data = fs.readFileSync('./kauan/darkjokes.js');
				...
				break
                case 'alerta':
				 data = fs.readFileSync('./kauan/alerta.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
         ...
         case 'moddroid':
			...
			kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
			await limitAdd(sender)
			break
		case 'happymod':
			...
			break
				...
				case 'naruto':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'naruto!!', quoted: mek })
					await limitAdd(sender)
					break 
				case 'nsfwboquete':
				...
				case 'minato':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'minato!!', quoted: mek })
					await limitAdd(sender)
					break 
				case 'boruto':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'boruto!!', quoted: mek })
					await limitAdd(sender)
					break 
				case 'animecry':
					cry = getRandom('.gif')
                   ...
                   if (!isGroup) return reply(mess.only.group)
					...
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
						buffer = fs.readFileSync(rano)
						kauankkk.sendMessage(from, buffer, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender) 
					break 
...
break
				...
				case 'hinata':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'hinata!!', quoted: mek })
					await limitAdd(sender)
					break 
           case 'hobby':
					...
					kauankkk.sendMessage(from, 'Questão : *'+hobby+'*\n\nResponda : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break
           case 'gay1':
		   ...
		   await limitAdd(sender)
				...
				case 'sasuke':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'sasuke!!', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'sakura':
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'sakura!!', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você quem é o dono? ')
					kauankkk.sendMessage(from, addfoto(prefix), text, { quoted: mek })
					break
			    case 'otagall3':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
			...
			case 'instaimg':
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    reply(mess.wait)
				    kauankkk.sendMessage(from, insta, image, {quoted: mek})
				    await limitAdd(sender) 
				    break  
				case 'instavid':
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    insta = getBuffer(anu.result)
				    reply(mess.wait)
				    kauankkk.sendMessage(from, insta, video, {mimtype: 'video/mp4', filename: 'instagram'.mp3, quoted: mek})
				    await limitAdd(sender) 
				    break  
				...
				break
                                case 'afk':
                                        ...
                                        kauankkk.sendMessage(from, tag, text, {quoted: mek})
                                        break 
				case 'indo10':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *.daftarvip* para comprar acesso Premium!' ,text, { quoted: mek })
					qute9 = await getBuffer(`https://i.ibb.co/z6w14Gq/32d64d819e21.png`)
					kauankkk.sendMessage(from, qute9, image, { quoted: mek, caption: 'Aqui está o Bro Baixe o seu próprio link\n\nhttps://www.mediafire.com/file/snwja297dv4zvtl/VID-20210107-WA0036.mp4/file' })
					break
case 'iri':
const irimp3 = fs.readFileSync('./assets/iri.mp3');
kauankkk.sendMessage(from, irimp3, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
				case 'bucin':
			    ...
			    case 'nsfwblowjob':
				    try {
						...
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não faça ingredientes para o tio comum'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
				...
				case 'testime':
					setTimeout( () => {
					kauankkk.sendMessage(from, 'O tempo acabou:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, 'Mais 5 segundos', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					kauankkk.sendMessage(from, '10 segundos para ir', text) // ur cods
					}, 0) // 1000 = 1s,
					break
		        case 'gay':		
					...
					kauankkk.sendMessage(from, 'Como você é gay: *'+rate+'*\n\nSua porcentagem gay : '+ kl+'%', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'delete':
					...
					if (mentioned.length > 1) {
						...
						kauankkk.deleteMessage(from, mentioned)
					} else {
				...
				case 'openanime':
					...
					if (Number(args[0]) === 1) {
						if (isAnime) return reply('O modo anime já está ativo')
						anime.push(from)
						fs.writeFileSync('./kauan/anime.json', JSON.stringify(anime))
...
reply('A mimir...')
exec(pm2, (err, stdout) => {
if(err) return kauankkk.sendMessage(from, "Erro", text, {quoted: mek})
if (stdout) {
kauankkk.sendMessage(from, stdout, text, {quoted: mek})
} 
})
break
...
break
				    ...
				    try {
						...
						buffer = await getBuffer(res.result)
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
				...
				case 'setfoto':
                    ...
                    if (!isGroupAdmins) return reply(mess.only.admin)
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await kauankkk.downloadAndSaveMediaMessage(mek)
                    await kauankkk.updateProfilePicture (from, media)
                ...
                case 'level':
					...
					resul = `┏━━❉ *LEVEL* ❉━━\n┣⊱ Nome : ${sem}\n┣⊱ Seu XP :  ${userXp}\n┣⊱ Seu Level : ${userLevel}\n┗━━━━━━━━━━━━`
					kauankkk.sendMessage(from, resul, text, { quoted: mek})
					.catch(async (err) => {
                    console.error(err)
                    await reply(`Error!\n${err}`)
                    })
				...
				case 'slide':
					...
					reply(mess.wait)
					kauankkk.sendMessage(from, atytyd, video, {quoted: mek})
					await limitAdd(sender) 
					break  
				case 'cpaper':
				cpaper = `${body.slice(8)}`
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						antilink.push(from)
						fs.writeFileSync('./kauan/antilink.json', JSON.stringify(antilink))
						reply('Grupo anti-link ativado com sucesso neste grupo ✔️')
						kauankkk.sendMessage(from,`Atencao a todos os membros ativos deste grupo anti-link. ee você enviar um link de grupo, voce sera expulso daqui  grupo`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O modo de grupo anti-link foi desabilitado ')
						var ini = anti.kauankkkOf(from)
				...
				case 'cgame':
					...
					kauankkk.sendMessage(from, buff, image, {caption: 'Aqui amigo (a)', quoted: mek})
					await limitAdd(sender) 
					break 
				case 'cparty':
					...
					kauankkk.sendMessage(from, bufferu, image, {caption: 'Aqui amigo (a)', quoted: mek})
					await limitAdd(sender) 
					break 
				case 'cstyle':
			    ...
			    case 'lovemake':
              	    if (args.length < 1) return reply('Onde está o texto, irmão??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return kauankkk.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buff = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=ANTIGRATISNIHANJENKKK`, {method: 'get'})
                    kauankkk.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	break
                	case 'tomp3':
                	kauankkk.updatePresence(from, Presence.composing) 
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						kauankkk.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
				case 'boquet':
				kauankkk.updatePresence(from, Presence.composing) 
                 ...
                 kauankkk.sendMessage(from, randBokep, image, {quoted: mek, caption: randTeks})
				await limitAdd(sender) 
				break
			    ...
			    case 'igstalk':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: capt})
					break
				//lgiproses
				case 'tesss':
					if (args.length < 1) return reply('o que você quer tio')
			    ...
			    case 'waifu':
				    try {
						...
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'ksksksks hmm'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'waifu2':
					...
					waifu = `*${anu.desc}`
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: waifu})
					break
				case 'text3d':
              	    if (args.length < 1) return reply('Onde está o texto, irmão??')
                    teks = `${body.slice(8)}`
				...
				case 'wibu':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: wibu})
					break
                   case 'map':
                   data = await fetchJson(`https://mnazria.herokuapp.com/api/maps?search=${body.slice(5)}`)
                   hasil = await getBuffer(data.gambar)
                   kauankkk.sendMessage(from, hasil, image, {quoted: mek, caption: `Resultados de *${body.slice(5)}*`})
                   await limitAdd(sender)
                   break
                   case 'covidcountry':
                   kauankkk.updatePresence(from, Presence.composing) 
                   ...
                   break
					...
					teks += `│+ Total de usuários : ${user.length}\n╰──────*「 *CAUSS* 」*────`
					 kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": user}})
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, qm e vc?')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await kauankkk.downloadAndSaveMediaMessage(encmedia)
						kauankkk.sendMessage(from, 'BOT DESLIGADO COM SUCESSO', text, { quoted: mek })
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							...
							.on('end', function () {
								...
								kauankkk.sendMessage(from, buffer222, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								kauankkk.sendMessage(from, buffer333, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								kauankkk.sendMessage(from, buffer555, sticker, {quoted: mek})
								fs.unlinkSync(ran)
							})
			...
			case 'closegc':
					...
					kauankkk.groupSettingChange (from, GroupSettingChange.messageSend, true);
					reply(close)
					break
				case 'kurumi':
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'kurumi chan!!', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'miku':
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'miku chan!!', quoted: mek })
					await limitAdd(sender) 
					break 
...
// recursos anime, random @Causs domina vida
				...
				kauankkk.sendMessage(from, jes, image,{quoted : mek, caption : 'DONE'})
				break
                case 'resepmasakan':
                ...
                reply(mess.wait)
                   ...
                   kauankkk.sendMessage(from, buff, image, {quoted: mek, caption: resep})
                   await limitAdd(sender) 
                   break 
               case 'cersex':
                   ...
                   kauankkk.sendMessage(from, sex, image, {quoted: mek, caption: cerita})
                   await limitAdd(sender) 
                   break 
              case 'cerpen':
                   ...
                   kauankkk.sendMessage(from, cerpen, text, {quoted: mek})
                   break 
               case 'puisiimg':

                   pus = await getBuffer(`https://api.vhtear.com/puisi_image&apikey=${VthearApi}`, {method: 'get'})
                   kauankkk.sendMessage(from, pus, image, {quoted: mek})
                   break 
				case 'akira':
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'akira chan!!', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'itori':
					reply(mess.wait)
					...
					kauankkk.sendMessage(from, nye, image, { caption: 'itori chan!!', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'kurumi':
					reply(mess.wait)
                ...
                case 'bukagc':
					...
					kauankkk.groupSettingChange (from, GroupSettingChange.messageSend, false)
					kauankkk.sendMessa
				case 'setppbot':
				kauankkk.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					...
					media = await kauankkk.downloadAndSaveMediaMessage(enmedia)
					await kauankkk.updateProfilePicture(botNumber, media)
				...
				kauankkk.sendMessage(from, film, text, {quoted: mek})
				await limitAdd(sender) 
					break					
				case 'randomcat':
					reply(mess.wait)
				...
				case 'mlherolist':
					...
					kauankkk.sendMessage(from, icon, image, {quoted: mek})
					break
			    case 'randomanime':
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/randomanime`, {method: 'get'})
						buffer = await getBuffer(res.result)
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni randomanime!'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'randomhentai':
				    try {
						...
						buffer = await getBuffer(res.result)
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'hentai teros'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
					break
                case 'irii':
			kauankkk.sendPtt(from, './lindy/iri2.mp3', id)
			break
                 case 'play':
                reply(mess.wait)
                play = body.slice(5)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'setnome':
			       ...
			       if (!isGroupAdmins) return reply(mess.only.admin)
			   	   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                   kauankkk.groupUpdateSubject(from, `${body.slice(9)}`)
                   kauankkk.sendMessage(from, 'Sucesso, alterou o nome do grupo', text, {quoted: mek})
                   break
				case 'galaxtext':
					if (args.length < 1) return reply('o que você quer tio')
					teks = body.slice(12)
					...
					break
                      case 'gay1': 
                                        var imgbb = require('imgbb-uploader')
                                         ...
                                         reply(mess.wait)
                                         owgi = await  kauankkk.downloadAndSaveMediaMessage(ger)
                                        ...
                                        anu1 = `https://some-random-api.ml/canvas/gay?avatar=${teks}`
                                         ...
                                         if (err) return reply(mess.error.stick)
                                                nobg = fs.readFileSync(rano)
                                                 kauankkk.sendMessage(from, nobg, sticker, {quoted: mek})
                                                fs.unlinkSync(rano)
                                        })
				...
				case 'party':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=ANTIGRATISNIHANJENKKK`)
					kauankkk.sendMessage(from, buffer, image, {caption: 'Aqui mana', quoted: mek})
					break
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
          ...
          reply('Espere..')
                ...
                case 'lionlogo':
                      ...
                      break
			    case 'fakereplay':
                   kauankkk.reply(from, 'ange mas', 'mending lari', "0816-5466368")
                   break
				case 'infogc':
				kauankkk.updatePresence(from, Presence.composing)
				if (!isGroup) return reply(mess.only.group)
					try {
					ppimg = await kauankkk.getProfilePicture(from)
				} catch {
					...
					kauankkk.sendMessage(from, buf, image, {quoted: mek, caption: teks})
					break
			    case 'owner':
                    kauankkk.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
                    kauankkk.sendMessage(from, 'Ctt do meu dono ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'info':
					...
					kauankkk.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
                case 'blowjob':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
				...
				case 'bloqueados':
					...
					kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
				case 'leens':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				case 'f':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								kauankkk.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								kauankkk.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
							})
						})
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
					...
					break
				case 'tts':
					if (args.length < 1) return kauankkk.sendMessage(from, 'Qual é o código da linguagem, tio?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return kauankkk.sendMessage(from, 'Cadê o texto tio', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							kauankkk.sendMessage(from, buff, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					break
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					kauankkk.sendMessage(from, ssha, image, {caption: 'Nihkkkak', quoted: mek})
					break
				case 'minion':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, minn, image, {caption: 'Nihhjmmak', quoted: mek})
					break
				case 'neon':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, nooe, image, {caption: 'Nihjjkkak', quoted: mek})
					break
				case 'neongreen': 
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, gree, image, {caption: 'Njkkkak', quoted: mek})
					break
				case 'neon2':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, duaa, image, {caption: 'kkkkkk', quoted: mek})
					break
				case '3d':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					kauankkk.sendMessage(from, tigaa, image, {caption: 'kkksk', quoted: mek})
					break
				case 'walpaperanime':
				    try {
			    	if (!isOwner) return reply(mess.only.ownerB)
						...
						kauankkk.sendMessage(from, bufferttt, image, {quoted: mek, caption: 'ksksks'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Kauan ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
					
					kapankah = body.slice(1)
					const elu =['1','2','3','4','5','6']
					const ule = elu[Math.floor(Math.random() * elu.length)]
					kauankkk.sendMessage(from, ule, text, { quoted: mek })
					break
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Kauan ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						kauankkk.sendMessage(from, mentioned)
					} else {
					...
					break
					case 'daftarvip': 
					kauankkk.sendMessage(from, daftarvip(prefix) , text, { quoted: mek })
					break
					case 'nekopoi':   
					
					kauankkk.sendMessage(from, nekopoi(prefix) , text, { quoted: mek })
					break
				...
				case 'sol':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://spundula.sirv.com/sunset.png?text.0.text=${teks}&text.0.position.gravity=north&text.0.position.y=26%25&text.0.size=23&text.0.color=000000&text.0.font.family=Playfair%20Display%20SC&text.0.font.weight=600&text.0.font.style=italic&text.0.background.opacity=5`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break	
                ...
                break
					...
					kauankkk.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": mod}})
					break
					case 'addpremium': 
					kauankkk.updatePresence(from, Presence.composing) 
                 ...
                 case 'calculadora':
				    ...
				    kauankkk.sendMessage(from, `*${anu.result.data}*`, text, {quoted: mek})
				    await limitAdd(sender) 	
				    break 
				case 'porno':
				    if (!isGroup) return reply(mess.only.group)
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'k'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.pinimg.com/originals/24/5f/91/245f91208f9030724dbc8d1bede2e9ff.gif`)
					kauankkk.sendMessage(from, buffer, sticker, {quoted: mek, caption: 'slc'})
					break
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/dPUVFF6.png`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '*_Comandos basicos para bot:_*\n\n*pkg upgrade && pkg update*\n*pkg install git*\n*git clone (link da git)*\n*cd (repositório)*\n*bash install.sh*\n*npm start*\n\n*CAUSS NO CONTROLE*'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'olha p isso mano, pqp '})
					break
				case 'akeno':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'loli1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/iphQUGi.jpg`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm, então quer ver loli?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra 2d 😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, vcs sao fodas ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde, rapeize 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite fml ❤️'})
					break
                case 'belle':                 
				 data = fs.readFileSync('./kauan/belle.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
				...
				case 'belle1':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '👀️'})
					break
				case 'lofi':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia ❤️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'termux':
					...
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Terminal é um programa muito conhecido no mundo das distribuições Linux. Ele é uma ferramenta que facilita muito nas tarefas relacionadas ao sistema. Agora, já pensou em utilizar o Terminal Linux no seu Android? Esta é a proposta do Termux.\n\n*TERMUX: UTILIZE O TERMINAL NO SEU ANDROID*\n\nA utilização do Terminal aumenta muito a produtividade do usuário que já possui um determinado nível de conhecimento técnico.\nCom o terminal, é possível fazer diversas e diferentes coisas, desde navegar entre os diretórios e instalar programas, até descompactar arquivos e monitorar os processos.'})
					break
		    	case 'grupoinfo':
                    kauankkk.updatePresence(from, Presence.composing)
                    if (!isGroup) return reply(mess.only.group)
                    ppUrl = await kauankkk.getProfilePicture(from) // leave empty to get your own
			        buffer = await getBuffer(ppUrl)
		            kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: `*NOME* : ${groupName}\n*MEMBRO* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESCRIÇÃO* : ${groupDesc}`})
                    break
				case 'meme':
					reply(mess.wait)
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					kauankkk.sendMessage(from, nye, image, { caption: 'cringe️', quoted: mek })
					await limitAdd(sender) 	
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				case 'rr':
                    ...
                    kauankkk.sendMessage(from, ''+ cu+'', text, { quoted: mek })
                    break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4V_4fdvF8rluX0T3KKGOvY0TusMwx7nVWtw&usqp=CAU`)
					kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* KAUAN\n*YOUTUBE:* https://youtube.com/channel/UCf4CRMZSQ1wysKBIJm3P8aw\n*WPP:* wa.me/+554491150998\n*INSTA:* @kauan_jardini\n\n\nEspero que tenham gostado do botzin️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'lolizinha'})
					})
					break
                                case 'lolih':
                                        gatauda = body.slice(6)
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						buffer = await getBuffer(res.url)
						kauankkk.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					})
					break
				case 'pronomeneu':
				...
				case 'yt2mp3':
					...
					kauankkk.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o url, hum?')
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
					kauankkk.sendMessage(from, buffer, video, {quoted: mek})
					break
				case 'tiktokstalk':
					try {
						if (args.length < 1) return kauankkk.sendMessage(from, 'Onde está o nome de usuário, hum', text, {quoted: mek})
			...
			reply(mess.wait)
				  ...
				  nama = tulis.split("/")[0];
					...
					kauankkk.sendMessage(from, nulis, image, {quoted: mek})
					await limitAdd(sender) 
					break  
				case 'url2img':
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					kauankkk.sendMessage(from, teks, text, {detectLinks: false, quoted: mek})
					break
				case 'limpar':
					if (!isOwner) return reply('Quem é Você?, Voce não é meu dono 😂')
					anu = await kauankkk.chats.all()
					kauankkk.setMaxListeners(25)
					for (let _ of anu) {
						kauankkk.deleteChat(_.jid)
					}
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await kauankkk.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							kauankkk.sendMessage(_.jid, buff, image, {caption: `[ TRANSMIÇÃO tropa kauan falando ]\n\n${body.slice(4)}`})
						}
        ...
        case 'promover':
				...
				case 'rebaixar':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Soldado rebaixado com sucesso\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						kauankkk.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					...
					try {
						...
						kauankkk.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Falha ao adicionar esse numero, talvez porque é privado')
					}
					break
				case 'banir':
					if (!isGroup) return reply(mess.only.group)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'soldado removido com sucesso :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'admins':
					...
					break
                                case 'linkgp':
                                        ...
                                        if (!isGroupAdmins) return reply(mess.only.admin)
                                        if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                                        linkgc = await kauankkk.groupInviteCode(from)
                                        reply('https://chat.whatsapp.com/'+linkgc)
                                        break
                                case 'leave':
                                        if (!isGroup) return reply(mess.only.group)
                                        if (isGroupAdmins || isOwner) {
                                            kauankkk.groupLeave(from)
                                        } else {
				...
				case 'toimg':
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadê o texto mano, hum?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso modo simi😗️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativou com sucesso modo simi😡️')
					} else {
						reply('1 para ativar, 0 para desativar, KKKKK')
					}
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
					if (!isadminbot) return reply(mess.only.admin)
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já esta ativo as boas vindas vc num lé as cvs não?')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Vc ativou as boas vindas️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativou as boas vindas️')
					} else {
						reply('1 para ativar, 0 para desativar, KAKKKK')
					}
                                      break
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					...
					try {
						pp = await kauankkk.getProfilePicture(id)
						buffer = await getBuffer(pp)
						kauankkk.updateProfilePicture(botNumber, buffer)
						mentions(`Foto do perfil atualizada com sucesso, usando a foto do perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('iiii fudeu, deu erro aqui, a pessoa deve estar sem foto 😔')
					}
					break
		        case 'setfoto2':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isadminbot) return reply('Quem é Você?kkkk')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await kauankkk.downloadAndSaveMediaMessage(mek)
                    await kauankkk.updateProfilePicture (from, media)
                    ...
                    break
                case 'bc2':
					if (!isadminbot) return reply('Quem é Você?kkkkk')
					if (args.length < 1) return reply('.......')
					anu = await kauankkk.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await kauankkk.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							kauankkk.sendMessage(_.jid, buff, image, {caption: `[ admin bot Broadcast ]\n\n${body.slice(4)}`})
						}
				...
				case 'hidetag2':
					...
					var group = await kauankkk.groupMetadata(from)
					...
					kauankkk.sendMessage(from, options, text)
					break
					//
				case 'setpp3':
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(mess.wait)
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await kauankkk.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
							kauankkk.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
				...
				default:
                  ...
                  reply(`Olá ${pushname},\n desculpe **${command}*\n não está registrado no meu *menu*\n digite **menu* e veja meus comandos`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[Ecktor]','blue'), 'Comando não registrado no Ecktor bot', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
