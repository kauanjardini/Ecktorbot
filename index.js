...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Kauan\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5544991150998:+55 44 91150998\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('connecting', () => {
		start('2', 'Espere um pouquinho...')
	})
	client.on('open', () => {
		success('2', 'Conectado, agora só aproveitar o bot')
	})
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu':
                    ...
                    client.sendMessage(from, help(prefix, time, kyun, pushname), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "* Ecktor-BOT na área*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                    break
				case 'help1':
				case 'menu1':
		...
		case 'iri':
			...
			if (args.length == 1) {
				await client.sendTextWithMentions(from, arqs[1] + ' é ' + lvpc + '% ' + gado + 'KKKKJ.')
			} else { await client.reply(from, `Você é ` + 'lvpc' + '% ' + 'gado' + ' KKKKJ.', id.Mentioned) }
			break
				...
				decoc = `${body.slice(10)}`
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[Kauan falando]\n\n${body.slice(4)}`})
						}
						reply('Feito')
					} else {
						for (let _ of anu) {
...
await client.sendMessage(from, `💥O @${mem.split('@')[0]} Não teve sorte💥\n*😈 Agora se prepare para as consequências 😈*`, text, {contextInfo: {"mentionedJid": [mem]}})
await client.groupRemove(from, [mem])
await client.sendMessage(from, `👻O @${mem.split('@')[0]} teve sua vida ceifada👻`)
}, 10000)
break	
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
		...
		case 'bpfont':
			...
			await client.get(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(body.slice(6))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				const attp = Buffer.from(response.data, 'binary').toString('base64')
				await client.sendImageAsSticker(from, attp, { author: config.author, pack: config.pack, keepScale: true })
			})
			break          
			...
			reply(mess.wait)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ TRANSMIÇÃO tropa kauan falando ]\n\n${body.slice(4)}`})
						}
